...
mutable struct MacCanvas <: AbstractWGPUCanvas
	title::String
	size::Tuple
	canvasContext
	needDraw
...
function getWindowId(canvas::MacCanvas)
	return nothing
end

function present(canvas::MacCanvas, textureView)
	return nothing
end

function getPreferredFormat(canvas::MacCanvas)
	return getEnum(WGPUTextureFormat, "RGBA8Unorm")
end

function defaultInit(::Type{MacCanvas})
...
function getContext(gpuCanvas::MacCanvas)
